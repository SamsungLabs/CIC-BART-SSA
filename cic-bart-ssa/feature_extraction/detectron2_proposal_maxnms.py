...
def doit(raw_image, gt_boxes, predictor):
    bb = np.array(gt_boxes[0])
    raw_boxes = Boxes(torch.from_numpy(bb).cuda())
...
def build_model():
    ...
    cfg.MODEL.WEIGHTS = str('/group-volume/IVU/flickr30k/faster_rcnn_checkpoints/faster_rcnn_from_caffe_attr.pkl')
...
def collate_fn(batch):
    ...
    for i, entry in enumerate(batch):
        ...
        bboxes.append(entry['bboxes'])

    batch_out = {}
    batch_out['img_ids'] = img_ids
    batch_out['imgs'] = imgs
    batch_out['bboxes'] = bboxes
            ...
            for i, batch in tqdm(enumerate(dataloader),
                ...
                try:
                    ...
                    if len(gt_bboxes[0]) == 0:
                        print('No bbox for image' + str(img_ids))
                        continue
                ...
                try:
                    instances, features = doit(img, gt_bboxes, detector)
