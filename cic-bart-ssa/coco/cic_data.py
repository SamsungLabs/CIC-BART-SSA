...
from utils import bb_intersection_over_union, coverage_computation
...
def get_datum(datum):
    ...
    cntl_names = datum['cntl_names']
    coverage = datum['coverage']
    sentence = datum['sentence']
    n_nongrounded_entities = datum['n_nongrounded_entities']
    ...
    if datum['ground_caption']:
        for i in range(args.ground_upsample):
            new_datum = {
                ...
                'cntl_names': cntl_names,
                'sentence': sentence,
                'n_nongrounded_entities': n_nongrounded_entities
            }
...
class PretrainDataset(Dataset): 
    def __init__(self, split='train', rank=-1, topk=-1, verbose=True, args=None, is_train=True, mode='train', raw_dataset=None):
        ...
        data = self.init_coco(split=split, args=args, is_train=is_train, mode=mode)

        if self.verbose:
            print("# of captions for CIC:", len(data))
    ...
    def init_coco(self, split='train', args=None, is_train=True, mode='train'):
        gt_source = 'coco'
        ...
        _data = json.load(f_cocoentities)

        if self.verbose:
            print(f"Loaded {len(_data)} data from", gt_source)
        ...
        for img_id in _data:
            ...
            if self.args.use_entities_data or self.args.use_ssa_data:
                try:
                    count = 0
                    for caption in _data[img_id]:
                        if _data[img_id][caption]['split'] != mode:
                            use_image = False
                            break

                        cntl_bboxes = []
                        cntl_names = ''
                        annotations = _data[img_id][caption]
                        n_nongrounded_entities = 0
                        for phrase in annotations['noun_chunks']:
                            phrase_category = phrase[1]
                            if phrase_category not in cntl_names:
                                cntl_names += ' ' + phrase_category
                            if phrase_category in annotations['detections']:
                                phrase_bboxes = annotations['detections'][phrase_category]
                                for bbox_id in phrase_bboxes:
                                    temp_box = bbox_id[1]
                                    temp_box = np.array([temp_box[0],temp_box[1],temp_box[2],temp_box[3]]).astype(dtype='float32')
                                    if len(cntl_bboxes) == 0:
                                        cntl_bboxes.append(temp_box)
                                    elif not np.any(np.all(temp_box == cntl_bboxes, axis=1)):
                                        cntl_bboxes.append(temp_box)
                            else:
                                n_nongrounded_entities += 1

                        if len(cntl_bboxes) == 0:
                            continue

                        if n_nongrounded_entities > 0:
                            if args.remove_nongrounded or mode == 'val' or mode == 'test':
                                continue
                            else:
                                print("You should not be here during testing")
                                cntl_bboxes.append(np.array([-1,-1,-1,-1]).astype(dtype='float32'))
                        ...
                        datum['cntl_names'] = cntl_names
                        datum['sentence'] = caption
                        if n_total_bboxes > 0 and n_cntl_bboxes > 0:
                            ...
                            continue
                        datum['n_nongrounded_entities'] = n_nongrounded_entities
            ...
            # if self.args.use_ssa_data and use_image:
            if self.args.use_ssa_data and ('train' in split) and use_image:
                count = 0
                try:
