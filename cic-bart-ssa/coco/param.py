...
def parse_args(parse=True, **optional_kwargs):
    ...
    # Data Splits
    parser.add_argument("--train", default='flickr-ent_train_split_ids.txt')
    parser.add_argument("--valid", default='flickr-ent_validation_split_ids.txt')
    parser.add_argument("--test", default='flickr-ent_testing_split_ids.txt')
    parser.add_argument('--test_only', action='store_true')

    parser.add_argument('--dataset_type', type=str, default='coco')
    parser.add_argument('--coco_entities', type=str, default='coco_entities_release.json') # COCO entities dataset
    parser.add_argument('--dataset_images', type=str, default='flickr30k/flickr30k-images/') # flickr 30k images folder 
    parser.add_argument('--precomp_features', type=str, default='precomputed_image_features.h5') # coco precomputed image feature vectors
    ...
    parser.add_argument('--load', type=str, default='', help='Load the model (usually the fine-tuned model).')
    ...
    parser.add_argument("--use_entities_data", action='store_true') # use original data for training/testing
    parser.add_argument("--use_ssa_data", action='store_true') # use ssa data for training/testing
    parser.add_argument("--remove_nongrounded", default=True, type=bool) # remove samples with non-grounded nouns "noisy labels"
    ...
    parser.add_argument("--ssa_dataset", default="", type=str) # sampled
    parser.add_argument("--original_control_dataset", default="flickr30k/cntlImgCaptioning_flickr30kCaptions/", type=str) # original flickr-30k dataset in ASG json format
    
    parser.add_argument('--diverse_length', action='store_true') # option for diverse fine-grained length sampling
    parser.add_argument('--add_verbs', action='store_true') # include ground truth verbs in control signal
    ...
    parser.add_argument("--results_gruen_csv", default="", type=str)
