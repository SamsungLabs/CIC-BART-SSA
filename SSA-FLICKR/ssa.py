...
from utils import load, bb_intersection_over_union
...
print ('Num of filtered edges %d'%(len(filtered_edges)))

def AMR_pipeline(body_file, flickr_sentences, flickr_annotations, output_sampledamrs, output_meta_amrs, output_nodevginfo, propbank, glove, glove_threshold=0.5, trim=False, hyper=None):
    '''
    :param body_corpus: input body corpus
    :return:
    '''
                            ...
                            if length_shortest_path < 3:
                                for idx in range(length_shortest_path-1):
                                    ...
                                    if ('ARG' not in edge) and ('mod' not in edge) and ('snt' not in edge) and ('subset' not in edge):
                                        ...
                                        # print('Remove path ' + str(shortest_path) + ' with role ' + edge)
        ...
        for n1 in nodes:
            for n2 in nodes:
                ...
                if n1 in ann.keys() and n2 in ann.keys():
                    id1 = ann[n1][0]['phrase_id']
                    id2 = ann[n2][0]['phrase_id']
                    if id1 != id2:
                        if id1 in ann[n1][1]['boxes'] and id2 in ann[n2][1]['boxes']:
                            bb_1 = ann[n1][1]['boxes'][id1]
                            bb_2 = ann[n2][1]['boxes'][id2]
                            bb_counter = 0
                            for bb1 in bb_1:
                                for bb2 in bb_2:
    ...
    def pairwise_merge(doc_nodes1, doc_root_nodes1, doc_edges1, doc_attrs1, ann1, doc_nodes2, doc_root_nodes2, doc_edges2, doc_attrs2, ann2, glove_vecs, sim_score):
        ...
        for node1 in doc_nodes1_init:
            ...
            for node2 in doc_nodes2_init:
                ...
                if 'and' == doc_nodes2_init[node2][0] and 'and' == doc_nodes1_init[node1][0]:
                    ...
                    if len(parents_node1) > 0 and len(parents_node2) > 0:
                        for par_1 in parents_node1:
                            for par_2 in parents_node2:
                                g_dist = -1
                                label1 = doc_nodes2_init[par_2][0].split('#^#')
                                label2 =  doc_nodes1_init[par_1][0].split('#^#')
                                for l1 in label1:
                                    for l2 in label2:
                                        temp = glove_cosine_similarity(l1, l2, glove_vecs)
                                        if temp > g_dist:
                                ...
                                if g_dist >= glove_threshold  and ('and' != doc_nodes1_init[par_1][0]) and ('and' != doc_nodes2_init[par_2][0]) :
                                    # print("Merge 'and' nodes for parent 1 " + doc_nodes1_init[par_1][0] + " and parent 2 " + doc_nodes2_init[par_2][0])
                                    keys[node2] = node2
                ...
                if 'and' == doc_nodes2_init[node2][0] or 'and' == doc_nodes1_init[node1][0]:
                    ind2 +=1
                    continue

                # visual grounding
                if node1 in ann1.keys() and node2 in ann2.keys():
                    phrase_id_1 = ann1_init[node1][0]['phrase_id']
                    phrase_id_2 = ann2_init[node2][0]['phrase_id']
                    try:
                        if phrase_id_1 in ann1_init[node1][1]['boxes'] and phrase_id_2 in ann2_init[node2][1]['boxes']:
                            n_bb_1 = len(ann1_init[node1][1]['boxes'][phrase_id_1])
                            n_bb_2 = len(ann2_init[node2][1]['boxes'][phrase_id_2])
                        else:
                            n_bb_1 = 1; n_bb_2 = 2
                    except:
                        n_bb_1 = 1
                        n_bb_2 = 2
                    if phrase_id_1 == phrase_id_2:
                        # print('Try to visual ground the nodes '+ doc_nodes1[node1][0] + ' and ' + doc_nodes2[node2][0])
                        if doc_nodes1[node1][0] == doc_nodes2[node2][0]:
                            # print("VISUAL GROUNDING MERGE for node 1 " + doc_nodes1_init[node1][0] + " and node 2 " + doc_nodes2_init[node2][0])  
                            keys[node2] = node2
                            common_counter += 1
                            ind2 += 1
                            continue
                        ...
                        bb_counter = 0
                        for bb1 in ann1_init[node1][1]['boxes'][phrase_id_1]:
                            for bb2 in ann2_init[node2][1]['boxes'][phrase_id_2]:
                                iou = bb_intersection_over_union(bb1, bb2)
                                if iou > 0.9:
                                    bb_counter += 1
                        if bb_counter >= n_bb_1:
                            g_sim = glove_cosine_similarity(doc_nodes1_init[node1][0], doc_nodes2_init[node2][0], glove_vecs)
                            if g_sim > 0.5:
                                print("Merge nodes based on their bounding box similarity!")
                                keys[node2] = node2
                                common_counter += 1
                                ind2 += 1
                                continue
                ...
                # merge same label nodes
                if doc_nodes1_init[node1][0] == doc_nodes2_init[node2][0]:
                    ...
                    if len(res) != 0:
                        ...
                        if '-' not in doc_nodes1_init[node1][0]:
                            tag = nltk.pos_tag(nt.sent_tokenize(doc_nodes1_init[node1][0]))
                            if tag[0][1] != 'JJ' and tag[0][1] != 'JJR' and tag[0][1] != 'JJS':
                                keys[node2] = node2
                                common_counter += 1
                        else:
                            keys[node2] = node2
                ...
                if dist[ind2,ind1] >= glove_threshold:
                ...
                if len(res1) > 0 and len(res2) > 0:
                    ...
                    g_sim = 1
                    label1 = doc_nodes1_init[node1][0].split('#^#')
                    label2 = doc_nodes2_init[node2][0].split('#^#')
                    for l1 in label1:
                        for l2 in label2:
                            temp = glove_cosine_similarity(l1.split('-')[0], l2.split('-')[0], glove_vecs)
                            if temp < g_sim:
                                g_sim = copy.copy(temp)
                            ...
                            if ':ARG1' in children_node1 and ':ARG1' in children_node2:
                                if children_node1[':ARG1'] == children_node2[':ARG1']:
                                    if ':ARG2' in children_node1 and ':ARG2' in children_node2:
                                        # g_sim = glove_cosine_similarity(doc_nodes1_init[node1][0].split('-')[0], doc_nodes2_init[node2][0].split('-')[0], glove_vecs)
    ...
    def save_amr_graph(pair_fname, f, doc_nodes, doc_edges, doc_attrs, doc_root_nodes, curr_filename, id, separate_meta_corpus, flag_remove_redundant_paths=False, glove_vecs = {}, ann = {}, sampling_fname = None):
        ...
        try:
            if flag_remove_redundant_paths == True:
                ...
                visuallyGround_AMRnodes(sub_nodes, ann, id)

                g_str, nodes, edges, doc_root_nodes = _graph(sub_nodes, sub_edges, sub_attrs, sub_root_nodes, hyper_path=hyper)
                g_str, doc_edges = sampling(sampling_fname, str(id), g_str, nodes, edges, doc_root_nodes)
                sub_graphs, disc = _disjointAMR(curr_filename, g_str, nodes, edges, doc_root_nodes)
                ...
                for i in range(5):
                    v_sepa = separate_meta_corpus[str(curr_filename.split('.')[0]) + '.' + str(i)]
                ...
                for r_k, r_g in sub_graphs.items():
                    # print ('---------r_k-----------', r_k)
                    g, root_nodes = r_g
                    '''
                    merged one
                    ...
                    for ind in range(5):
                        ...
                        # pair_fname.write(' '.join(['# ::source'] + [new_meta['source']]) + '\n')
    ...
    def sample_from_predicate_node(pred_node, g_str, filterPredRoles):
        flag = True
        sampled_graph = []
    ...
    def sampling(fname, img_id, g_str, doc_nodes, doc_edges, doc_root_nodes):

        def save_sampled_graph(fname, sampled_graph, list_predicates, pred, role_idx):
            g = Graph(sampled_graph)
            pen_sampled_graph = penman.encode(g)

            fname.write(' '.join(['# ::status'] + ['ok']) + '\n')
            fname.write(' '.join(['# ::id'] + [img_id.split('.')[0] + '_' + list_predicates[pred][0] + '_' + list_predicates[pred][2]]) + '_' + str(role_idx) + '\n')
            fname.write(' '.join(['# ::snt-type'] + [list_predicates[pred][2]]) + '\n')
            fname.write(' '.join(['# ::snt'] + [list_predicates[pred][2]]) + '\n')
        ...
        for ii in range(len(g_str)):
            if g_str[ii][1] == 'instance':
                ...
                if len(res) > 0:
                    ...
                    tag = nltk.pos_tag(nt.sent_tokenize(label))
                    if 'NN' in tag[0][1]:
                        list_nouns[ii] = g_str[ii]
            ...
            try:
                ...
                if pred_name + '.xml' in propbank_predicate_list:
                    ...
                    if len(pred_ARGs) > 0:
                        ...
                        sampled_graph = sample_from_predicate_node(list_predicates[pred], g_str, AMR_ARGx_list[:3])
                        if sampled_graph != None:
                            save_sampled_graph(fname=fname, sampled_graph=sampled_graph,list_predicates=list_predicates, pred=pred, role_idx=counter)
                        counter += 1

                        if AMR_ARGx_list[3] in pred_ARGs:
                            sampled_graph = sample_from_predicate_node(list_predicates[pred], g_str, AMR_ARGx_list[:4])
                            if sampled_graph != None:
                                save_sampled_graph(fname=fname, sampled_graph=sampled_graph,list_predicates=list_predicates, pred=pred, role_idx=counter)
                            counter += 1
                        if AMR_ARGx_list[4] in pred_ARGs:
                            sampled_graph = sample_from_predicate_node(list_predicates[pred], g_str, AMR_ARGx_list[:5])
                            if sampled_graph != None:
                                save_sampled_graph(fname=fname, sampled_graph=sampled_graph,list_predicates=list_predicates, pred=pred, role_idx=counter)
                            counter += 1
                        if AMR_ARGx_list[5] in pred_ARGs:
                            sampled_graph = sample_from_predicate_node(list_predicates[pred], g_str, AMR_ARGx_list)
                            if sampled_graph != None:
                                save_sampled_graph(fname=fname, sampled_graph=sampled_graph,list_predicates=list_predicates, pred=pred, role_idx=counter)
                            counter += 1

                    sampled_graph = sample_from_predicate_node(list_predicates[pred], g_str, None)
                    if sampled_graph != None:
                        save_sampled_graph(fname=fname, sampled_graph=sampled_graph,list_predicates=list_predicates, pred=pred, role_idx=counter)
                    counter += 1
            except Exception as e:
    ...
    def visuallyGround_AMRnodes(doc_nodes, ann, id):
        ...
        for node in doc_nodes:
            if node in ann:
                node_ann = ann[node]
                node_phraseID = node_ann[0]['phrase_id']
                if node_phraseID in node_ann[1]['boxes']:
                    node_bboxes =  str(node_ann[1]['boxes'][node_phraseID])
                    nodes_bboxes[str(node) + '_' + doc_nodes[node][0]] = node_bboxes
                elif node_phraseID in node_ann[1]['scene']:
                    nodes_bboxes[str(node) + '_' + doc_nodes[node][0]] = 'scene'
        with open(f"{output_nodevginfo}{id.split('.')[0]}.json", "w") as outfile:
    ...
    print('Loading caption amrs')
    body_corpus, amr_text, annotations = load(body_file, flickr_sentences=flickr_sentences, flickr_annotations=flickr_annotations, trim=trim, grouped_edges=grouped_edges,caption_type='less')
...
if __name__ == '__main__':
    ...
    AMR_pipeline(args.caption_amrs, args.flickr_sentences, args.flickr_annotations, args.sampledamrs, args.meta_amrs, args.nodevisualgroundinfo, args.propbank, args.glove, args.glove_threshold)